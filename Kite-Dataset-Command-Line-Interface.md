...
The schema produced by this command will be a record based on the first few lines of the file. If the first line is a header, it will be used to name the fields.
...
| `--require`              | Mark a field as required; the schema for this field will not allow null values.<br />May be used more than once. |
...
Build a schema from a Java class. Fields are assumed to be nullable if they are Objects and required if they are primitives. You can edit the generated schema directly to remove the `null` option for specific fields.
...
The CSV header is used to match CSV columns with the target record schema's fields, by name. If a header is not present (`--no-header` is used), then CSV columns will be matched with the target fields by position.

Values are validated using the target field's schema as each record is constructed. Invalid values in numeric fields and null values in required fields will cause exceptions. The empty string is handled as a null value for numeric fields.
